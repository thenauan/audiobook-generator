...
2. You can download the **`Installer.exe`** file from the Releases tab or run the `instalar_e_rodar.bat` script.
3. If using the installer, simply run it! It will silently verify, download, and install Node.js and Python (if missing), install the app's dependencies, and create a shortcut on your Desktop.
4. A shortcut will be created on your Desktop. Just open and use!
...
2. Grant execution permission to the installation script:
   ```bash
   chmod +x linux_mac/install_and_run.sh
   ```
3. Run the script:
   ```bash
   ./linux_mac/install_and_run.sh
   ```